...
export const ComparisonPage: React.FC = () => {
  const [secondDocument, setSecondDocument] = useState<string | null>(null)
  const [isComparing, setIsComparing] = useState(false)
  const [comparisonResult, setComparisonResult] = useState<{doc1: string, doc2: string, changes: number, status: string} | null>(null)
  const navigate = useNavigate()

  const handleSecondDocSelect = () => {
    setSecondDocument("Non-Disclosure Agreement 2024.pdf")
  }
  ...
  const handleStartComparison = () => {
    ...
    setTimeout(() => {
      ...
      setComparisonResult({
        doc1: 'Employment Agreement 2024.pdf',
        doc2: secondDocument || '',
        changes: 15,
        status: 'completed'
      })
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        ...
        <Button variant="legal" onClick={handleNewComparison}>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div 
                className="border-2 border-dashed border-muted rounded-lg p-6 text-center hover:border-legal-blue transition-colors cursor-pointer"
                onClick={handleSecondDocSelect}
              >
                {secondDocument ? (
                  <>
                    <div className="w-10 h-10 bg-legal-blue/10 rounded-lg flex items-center justify-center mx-auto mb-3">
                    ...
                    <p className="text-sm font-medium">{secondDocument}</p>
                    <p className="text-xs text-muted-foreground">Selected</p>
                  </>
                ) : (
                  <>
            ...
            <Button variant="legal" className="w-full" disabled={!secondDocument || isComparing} onClick={handleStartComparison}>
            ...
            <div className="text-center py-12 space-y-4">
              {comparisonResult ? (
                <>
                  ...
                  <div>
                    <h3 className="font-semibold text-lg">Comparison Complete</h3>
                    <p className="text-muted-foreground">Found {comparisonResult.changes} differences between the documents</p>
                  </div>
                  <Button variant="outline" className="mt-4" onClick={() => navigate('/comparison/new')}>
            ...
            {[
              ...
              <div key={comparison.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => navigate(`/comparison/${comparison.id}`)}>
                    View Results
                  </Button>
                </div>
              </div>
